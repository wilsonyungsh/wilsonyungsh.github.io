...
首頁跟作品集頁右上角（語言切換鈕旁邊）有一個月亮/太陽圖示的按鈕，可以手動切換深色模式；沒手動切換過的話，會跟著瀏覽器/系統的深色模式設定走。選擇會存在 `localStorage` 的 `wy_theme`（跟語言選擇的 `wy_lang` 是分開存的）。

顏色全部是 CSS 變數（`--bg`、`--text-1`、`--accent` 之類的），深色版本定義在 `build.R` 的 `:root[data-theme="dark"]` 區塊裡，跟淺色版本（`:root`）一一對應。**之後如果要加新的顏色/元件樣式，記得同時想好深色版本要長怎樣**，不要直接寫死 hex 色碼，不然深色模式下會漏改。

`career_map.html` 本身已經是深色設計，不受這個切換影響（它是獨立嵌入的 widget，本來就跟外層頁面的主題脫鉤）。
